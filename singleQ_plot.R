...
singleQ_plot <- function(dat, meta, nsize, color) {
  ...
  # palette for Rank Order
  rankorder_pal <- c("#115f9a", "#1984c5", "#22a7f0", "#48b5c4", "#76c68f", "#a6d75b", 
               "#c9e52f", "#d0ee11", "#d0f400")
  # palette for Matrix
  matrix_pal <- c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF", "#7AA6DCFF",
  ...
  if (meta$question_type %in% c("MC", "TE_AGE", "Slider")) {
    ...
    gt_tbl <- singleQ_table(dat, meta, nsize)
    
    data_plot <- list(data = pdat, p1 = p1, p1_flip = p1_flip, gt_tbl = gt_tbl)
    ...
    p1 <- ggplot(pdat, aes(x = choice_text, y = frac, group = factor(value), fill = factor(value))) +
      ...
      scale_fill_manual(values = rankorder_pal, name = "Rank") +
      scale_y_continuous(labels = scales::percent) +
    ...
    # check to see if matrix question is statement comparison and use palette
    if (meta$selector_type == "Bipolar") {
      ...
      # single plot for bipolar question type
